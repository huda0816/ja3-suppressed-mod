...
function SuppressionMeter:LogRemovedAps(target, consumed_ap)

	if not target or IsPlayerEnemy(target) then
		return
	end

	local logLevel = SuppressedGetModOptions("consumed_aps_log_level", "short", "string")

	CombatLog(logLevel, T({
		Untranslated("<em><target></em> lost <em><consumed_ap></em> AP due to suppression"),
		target = target:GetDisplayName(),
		consumed_ap = consumed_ap
	}))
end

function SuppressionMeter:LogPanicked(target, status)
	...
	CombatLog(logLevel, T({
		Untranslated("<em><target></em> became <status> due to suppression and got 12 AP"),
		target = target:GetDisplayName(),
...
function SuppressionMeter:HandleSuppression(target)
	...
	elseif self:PanicRoll(target, 120) then
		...
		self:LogPanicked(target, "Berserk")
	else
...
function SuppressionMeter:ApplySuppression(obj, added_supp, attacker)
	...
	if obj.team and obj.team.player_team then
		added_supp = MulDivRound(added_supp, self.SuppressionPlayerMultiplier, 100)
	else
		added_supp = MulDivRound(added_supp,self.SuppressionAiMultiplier, 100)
	end
...
function SuppressionMeter:LogToSnype(target, attacker, added_supp, near_miss, is_target, bruttosupp,
	...
	local hit = near_miss and Untranslated("not directly hit") or Untranslated("directly hit")
	...
	CombatLog("debug", T({
		...
		modifier = tostring(self.SuppressionMultiplier),
		pmodifier = tostring(self.SuppressionPlayerMultiplier),
		aimodifier = tostring(self.SuppressionAiMultiplier)
	}))
	CombatLog("debug", T({
		TLookupTag("<em>-----------</em>")
	}))
